...
def _record(domain, skill, diff, question, extra):
    assert question not in stems, question
    stems.add(question)
    n = len(questions) + 1
    q = {"id": f"mth-{n:04d}", "section": "math", "domain": domain, "skill": skill,
         "difficulty": diff, "question": question}
    q.update(extra)
    questions.append(q)
...
for i, (a, b, c) in enumerate([(5, 12, 13), (8, 15, 17), (7, 24, 25), (3, 4, 5), (20, 21, 29), (9, 40, 41)]):
    ratio = ["sin", "cos", "tan"][i % 3]
    val = {"sin": F(a, c), "cos": F(b, c), "tan": F(a, b)}[ratio]
    stem = (f"In right triangle PQR, angle R is the right angle, PR = {b}, QR = {a}, and PQ = {c}. What is the value of {ratio} P?")
    why = (f"For angle P, the opposite side is QR = {a}, the adjacent side is PR = {b}, and the hypotenuse is PQ = {c}. "
           f"{ratio} P = {num(val)}.")
...
for i, (r, ang) in enumerate([(6, 60), (9, 120), (12, 30), (10, 72), (8, 135), (15, 48)]):
    if i % 2 == 0:
        val = F(ang, 360) * 2 * r
...
for i, (a, b, s) in enumerate([(4, 6, F(5, 2)), (6, 9, F(4, 3)), (5, 8, 3), (9, 12, F(2, 3))]):
    ab2 = a * s
    val = b * s
...
# D7: angles
for i, n in enumerate([5, 6, 8, 10, 12]):
    total = (n - 2) * 180
    each = F(total, n)
    name = {5: "pentagon", 6: "hexagon", 8: "octagon", 10: "decagon", 12: "dodecagon"}[n]
...
for i, (a, v) in enumerate([(28, "0.47"), (35, "0.82"), (53, "0.60"), (17, "0.29")]):
...
if __name__ == "__main__":
    ...
    n_spr = sum(1 for q in questions if q.get("type") == "spr")
    print(f"{len(questions)} math questions ({n_spr} typed-answer, {len(questions) - n_spr} multiple choice) -> {os.path.relpath(OUT)}")
    for dom, c in Counter(q["domain"] for q in questions).items():
        print(f"  {c:4d}  {dom}")
    print("  difficulty:", dict(Counter(q["difficulty"] for q in questions)))
    print("  answer slots:", dict(sorted(Counter(q["answer"] for q in questions if "answer" in q).items())))
